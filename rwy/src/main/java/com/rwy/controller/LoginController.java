...
public class LoginController {
	...
	public Object login(User user, HttpSession session) {
		...
		if(users.getLoginname().equals(user.getLoginname()) && users.getLoginpassword().equals(user.getLoginpassword())) {
			...
			session.setAttribute(Constants.LOGIN_USER, users);
		}else if(!user.getLoginpassword().equals(users.getLoginpassword()) || user.getLoginname().equals(users.getLoginpassword())){
			msg = "201";
			map.put("msg", msg);
		}else {
			msg = "500";
